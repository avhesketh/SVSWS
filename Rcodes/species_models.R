...
balanus_time <- balanus %>% 
  group_by(colour, timediff, block) %>% 
  summarize(mean_count = mean(count))

balanus_trt <- ggplot(aes(x = timediff, y = mean_count, colour = colour),
                      data = balanus_time) +
  geom_point() +
  facet_wrap(~block)
...
balanus_black <- balanus %>% 
  filter(colour == "warm") %>% 
  filter(timediff == max(timediff))
balanus_white <- balanus %>% 
  filter(colour == "ambient") %>% 
  filter(timediff == max(timediff))

hist(balanus_white$count, breaks = 100)
hist(balanus_black$count, breaks = 100)
